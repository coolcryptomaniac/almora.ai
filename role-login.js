...
function friendly(err){const code=err?.code||'';if(code.includes('popup-closed'))return 'Google sign-in was closed before completion.';if(code.includes('account-exists'))return 'This email already uses another sign-in method.';if(code.includes('invalid-credential'))return 'Email or password was not accepted.';if(code.includes('weak-password'))return 'Use a stronger password with at least 6 characters.';return 'Sign-in could not be completed. Please try again.'}
async function authorize(user){if(role==='government'){const allowed=await getDoc(doc(db,'moderators',user.uid));if(!allowed.exists()){await signOut(auth);throw new Error('not-authorized-government')}return destinations.government}if(role==='business'){await setDoc(doc(db,'userRoles',user.uid),{business:true,lastSeenAt:serverTimestamp()},{merge:true});return destinations.business}await setDoc(doc(db,'userRoles',user.uid),{resident:true,lastSeenAt:serverTimestamp()},{merge:true});return destinations.resident}
