const slides=[
 ...
 {src:'./assets/generated/almora-twilight.webp',label:'Almora twilight · Aipan frame',kind:'AI-generated Almora.ai artwork'},
 {src:'./assets/generated/almora-sunset.webp',label:'Almora golden hour · Aipan frame',kind:'AI-generated Almora.ai artwork'},
...
function mountHeroSlideshow(){
 ...
 function paint(s){base.style.backgroundImage=`url('${s.src}')`;base.dataset.slide=String(index);caption.innerHTML=`<b>${s.label}</b><small>${s.kind}</small>`;dots.forEach((d,i)=>d.classList.toggle('active',i===index));requestAnimationFrame(()=>base.classList.remove('changing'))}
 function show(next){index=(next+slides.length)%slides.length;const s=slides[index];base.classList.add('changing');const probe=new Image();probe.onload=()=>paint(s);probe.onerror=()=>{console.warn('Hero slide unavailable:',s.src);base.classList.remove('changing');index=0;paint(slides[0])};probe.src=s.src}
