...
function mountHeroSlideshow(){
 ...
 function paint(s){setImage(s.src);base.dataset.slide=String(index);caption.innerHTML=`<b>${s.label}</b><small>${s.kind}</small>`;dots.forEach((d,i)=>d.classList.toggle('active',i===index));requestAnimationFrame(()=>base.classList.remove('changing'))}
