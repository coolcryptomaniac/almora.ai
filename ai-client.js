...
- For compound problems use this sequence: identify intent → assign specialist → check shared evidence lessons → inspect ranked local context → make an ordered plan → verify risky/current claims → answer.
...
async function sharedContext(agent){const [learnRes,contentRes]=await Promise.all([safeJSON('./data/agent-learning.json',{fallback:{lessons:[]},cacheKey:'agent-learning',maxAge:70*60*1000,retries:1,timeout:4500}),safeJSON('./data/content-feed.json',{fallback:{items:[],agentIndex:{}},cacheKey:'content-feed',maxAge:70*60*1000,retries:1,timeout:4500})]);const lessons=(learnRes.value?.lessons||[]).filter(x=>(x.toAgents||[]).includes(agent)||(x.fromAgents||[]).includes(agent)||agent==='town-concierge').slice(0,5);const feed=contentRes.value||{items:[],agentIndex:{}};const ids=new Set(feed.agentIndex?.[agent]||feed.agentIndex?.['town-concierge']||[]);const items=(feed.items||[]).filter(x=>ids.has(x.id)).slice(0,6);let out='';if(lessons.length)out+=`\n\nSHARED EVIDENCE LESSONS (coordination guidance, not model training):\n${lessons.map(x=>`- ${x.rule} Evidence: ${(x.evidence||[]).slice(0,2).join(' | ')}`).join('\n')}`;if(items.length)out+=`\n\nRANKED LOCAL CONTEXT (source-linked public content):\n${items.map(x=>`- [${x.category}] ${x.title}: ${x.summary}${x.url?` Source: ${x.url}`:''}`).join('\n')}`;return out}
async function askAlmoraAI(prompt,context=''){
 if(!aiReady||!primaryModel)throw new Error('AI Logic unavailable');const agent=routeAgent(prompt),shared=await sharedContext(agent);const grounded=`ACTIVE AGENT: ${agent}\n\n${context||'VERIFIED PUBLIC DATA: No relevant verified records supplied.'}${shared}\n\nUSER REQUEST:\n${prompt}`;
