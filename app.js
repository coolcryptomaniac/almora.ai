...
async function ask(q=promptBox.value){q=q.trim();if(!q)return;promptBox.value=q;answer.hidden=false;askButton.disabled=true;askButton.textContent='Thinking…';answer.innerHTML='<b>✦ Almora AI</b><p>Working on it…</p>';try{if(aiReady){const text=await askAlmoraAI(q,'Firestore is connected. The public map may include verified public collections and clearly-labelled demo markers. Current weather is fetched separately from Open-Meteo.');answer.innerHTML=`<b>✦ Almora AI</b><p>${esc(text).replace(/\n/g,'<br>')}</p><small>Firebase AI Logic · App Check ${appCheckReady?'active':'not initialized'}</small>`;}else{throw new Error('AI unavailable')}}catch(e){const f=fallback(q);answer.innerHTML=`<b>✦ ${f.title}</b><p>${f.body}</p><small>${firebaseReady?'Connected to Almora Firestore.':'Prototype mode · Firebase unavailable.'} AI fallback active.</small>`;}finally{askButton.disabled=false;askButton.textContent='Ask AI ↑';answer.scrollIntoView({behavior:'smooth',block:'nearest'});}}
askButton.onclick=()=>ask();document.querySelectorAll('[data-q]').forEach(el=>el.onclick=()=>ask(el.dataset.q));
