import { firebaseReady, addTownReport, watchCollection } from './firebase-client.js';
...
function ask(q=promptBox.value){q=q.trim();if(!q)return;promptBox.value=q;const hit=intents.find(x=>x[0].test(q));const title=hit?hit[1]:'Town concierge',body=hit?hit[2]:'I can route this to the right Almora workflow. Include the broad location, what happened and what outcome you need.';answer.hidden=false;answer.innerHTML=`<b>✦ ${title}</b><p>${body}</p><small>${firebaseReady?'Connected to Almora Firestore.':'Prototype mode · Firebase unavailable.'}</small>`;answer.scrollIntoView({behavior:'smooth',block:'nearest'});}
...
function addLiveMarker(kind,item,label){const lat=Number(item.lat??item.latitude??item.location?.lat),lng=Number(item.lng??item.longitude??item.location?.lng);if(!Number.isFinite(lat)||!Number.isFinite(lng))return;const title=item.title||item.name||item.role||label;const status=item.status||item.description||item.route||'Verified public record';const m=L.circleMarker([lat,lng],{radius:9,weight:2,fillOpacity:.9}).bindPopup(`<b>${title}</b><br>${status}<br><small>Firestore · verified/public collection</small>`);m.kind=kind;liveMarkers.push(m);if(visible(m))m.addTo(map)}
if(firebaseReady){
 const stores={publicIssues:'road',facilities:'health',jobs:'jobs',transport:'transport'};const snapshots={};
 Object.entries(stores).forEach(([name,kind])=>watchCollection(name,rows=>{snapshots[name]=rows;clearLive();Object.entries(stores).forEach(([collectionName,k])=>(snapshots[collectionName]||[]).forEach(item=>addLiveMarker(k,item,collectionName)));if(liveMarkers.length){seedMarkers.forEach(m=>map.removeLayer(m));document.querySelector('.mapNote').textContent='● Live map & weather   ● Firestore public data';}else{applyFilter();}},err=>console.warn(`Could not read ${name}`,err)));
}
...
document.querySelector('#submitReport').onclick=async(e)=>{e.preventDefault();const payload={category:document.querySelector('#category').value,location:document.querySelector('#reportLocation').value.trim(),description:document.querySelector('#reportText').value.trim(),status:'new',source:'resident',createdAt:new Date().toISOString()};if(!payload.location||!payload.description){document.querySelector('#reportStatus').textContent='Please add a broad location and description.';return}try{if(firebaseReady){await addTownReport(payload);document.querySelector('#reportStatus').textContent='Report submitted to Almora Firestore for moderation.';}else{const local=JSON.parse(localStorage.getItem('almora_reports')||'[]');local.push(payload);localStorage.setItem('almora_reports',JSON.stringify(local));document.querySelector('#reportStatus').textContent='Saved locally because Firebase is unavailable.';}setTimeout(()=>dialog.close(),1400)}catch(err){document.querySelector('#reportStatus').textContent='Firestore rejected the write. Deploy the repository Firestore rules, then retry.';console.error(err)}};